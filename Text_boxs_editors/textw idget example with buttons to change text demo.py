...
class App(object):
    def __init__(self, master):
        ...
        self.text.config(
            wrap="word"
        )  # wrap = 'word' or 'none' or 'char' - default is 'char'

        self.btn0 = ttk.Button(
            self.master,
            text="Insert random Text in TExt field",
            command=self.randomTextEntry,
        )
        self.btn0.pack()

        self.btn1 = ttk.Button(
            self.master, text="Get all text in line 1", command=self.getline1text
        )
        self.btn1.pack()

        self.btn2 = ttk.Button(
            self.master, text="Get All Text", command=self.getAllText
        )
        self.btn2.pack()

        self.btn3 = ttk.Button(
            self.master,
            text='Insert "My Name is \nAdi" on 3rd Line',
            command=self.putTextat3rdLine,
        )
        self.btn3.pack()

        self.btn4 = ttk.Button(
            self.master, text="Delete First Character of Text", command=self.deleteFirst
        )
        self.btn4.pack()

        self.btn5 = ttk.Button(
            self.master, text="Delete from 5 to 7 of line 3", command=self.delete5to8on3
        )
        self.btn5.pack()

        self.btn6 = ttk.Button(
            self.master, text="Delete first 3 lines", command=self.deleteFirst3Lines
        )
        self.btn6.pack()

        self.btn7 = ttk.Button(
            self.master,
            text="Replace line 1 with random text",
            command=self.replaceRandom,
        )
        self.btn7.pack()

        self.btn8 = ttk.Button(
            self.master, text="Disable Text Field", command=self.disableEnable
        )
        self.btn8.pack()
        ...
        self.btn9 = ttk.Button(
            self.master, text="Create tag named 'myTag' at line 2", command=self.tagDemo
        )
        self.btn9.pack()
    ...
    def tagDemo(self):
        if self.btn9["text"] == "Create tag named 'myTag' at line 2":
            ...
            self.btn10 = ttk.Button(
                self.master,
                text="Change myTag background to yellow",
                command=self.tagbgyellow,
            )
            self.btn10.pack()

            self.btn11 = ttk.Button(
                self.master,
                text="Remove tag from 1st word of line 2",
                command=self.tagrm21word,
            )
            self.btn11.pack()

            self.btn12 = ttk.Button(
                self.master, text="myTag Span", command=self.getTagSpan
            )
            self.btn12.pack()

            self.btn13 = ttk.Button(
                self.master,
                text="Show all Tags in Text widget",
                command=self.displayAllTags,
            )
            self.btn13.pack()
...
"""
    ...
    def putTextat3rdLine(self):
        self.text.insert(
            "1.0 + 2 lines lineend", ". My Name is \nAdi. And Yo \nAnd Yo..."
        )
