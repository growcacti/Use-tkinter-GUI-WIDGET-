...
class entryApp:
    def __init__(self, master):
        ...
        self.btn2 = ttk.Button(
            master,
            text="{} Text in Entry Field".format(self.crypt.get()),
            command=self.changecrypt,
        )
        self.btn2.pack()
    ...
    def getEntry(self):
        self.tkstrvar.set(
            self.entry.get()
        )  # get entry widget content and store it in tk_string variable
        self.msg.config(text=self.tkstrvar.get())  # set msg as value of string variable

    def btn1func(self):
        self.entry.delete(
            0, tk.END
        )  # delete all from 0 to END character in Entry Widget
