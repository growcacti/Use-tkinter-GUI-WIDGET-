...
def open_file(event=None):
    ...
    if input_file_name:
        ...
        win.title("{} - {}".format(os.path.basename(file_name), PROGRAM_NAME))
        content_text.delete(1.0, END)
...
def find(event=None):
    search_top_level = Toplevel(win)
    search_top_level.title("Find Text")
    search_top_level.transient(win)
...
content_text = Text(win, wrap="word", undo=1)
...
shortcut_bar = Frame(win, height=25, background="light sea green")
...
win.protocol("WM_DELETE_win", quit)
