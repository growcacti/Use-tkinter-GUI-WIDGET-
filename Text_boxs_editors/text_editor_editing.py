...
def mainWindow():
    ...
    def compare_files():
        ...
        messagebox.showinfo(
            "Plagiarism", f"The contents are {similarity * 100:.3f}% common."
        )
    ...
    def search_output(
        needle, if_ignore_case, content_text, search_toplevel, search_box
    ):
        content_text.tag_remove("match", "1.0", END)
        matches_found = 0
        if needle:
    ...
    def insertCartoon():
        try:
            select_photo = filedialog.askopenfilename(
            ...
            edge_image = cv.adaptiveThreshold(
                blur_image, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, 5, 3
            )
            ...
            if tkinter.messagebox.askyesno(
                "Save", "Do you want to save your cartooned image?"
            ):
                input_file_name = tkinter.filedialog.asksaveasfilename(
    ...
    file_menu.add_command(
        label="Compare File", image=compare_icon, compound="left", command=compare_files
    )
    file_menu.add_separator()
    file_menu.add_command(
        label="Exit",
    ...
    insert_menu.add_separator()
    insert_menu.add_command(
        label="Add Date", image=add_date_icon, compound="left", command=addDate
    )
    # end of Insert Menu
    ...
    speech_menu.add_command(
        ...
        image=speech_to_text_icon,
        compound="left",
        command=speech_to_text,
    )
    speech_menu.add_command(
        label="Import Audio",
        image=import_audio_icon,
        compound="left",
        command=importAudioFlie,
    )
    ...
    view_menu.add_command(
        label="View Your Cartoon",
        image=cartoon_icon,
        compound="left",
        command=insertCartoon,
    )
    ...
    for k in sorted(font_sizes):
        font_size.add_radiobutton(
            label=k, compound="left", variable=size_var, command=SetFontSize
        )

    for j in sorted(font_faces):
        font_face.add_radiobutton(
            label=j, compound="left", variable=face_var, command=SetFontFace
        )
