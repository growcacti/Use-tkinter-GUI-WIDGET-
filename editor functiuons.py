...
def AdjustBrightness(c1,image,root,event=None):
 master=Toplevel(root)

 def scaleRatio(val):
  return int(val)/50

 def getScaleValue(val):
  val1=scaleRatio(val)
  w,h=Image.open(image).size
  im=Image.open(image)
  enhancer=ImageEnhance.Brightness(im)
  img=enhancer.enhance(val1)
 ...
 def saveImage():
  im=Image.open(image)
  enhancer=ImageEnhance.Brightness(im)
  img=enhancer.enhance(scaleRatio(Sa.get()))
  misc.imsave("1.jpg",img)
  master.destroy()

 def cancelFunction():
  im=Image.open("1.jpg")
  master.img2=ImageTk.PhotoImage(im)
  c1.delete("all")
  c1.create_image(canvas_width/2,canvas_height/2+10,anchor="center",image=master.img2)
  master.destroy()
 ...
 Sa=Scale(frame1,length=200,from_=0,to=100,orient=HORIZONTAL,command=getScaleValue,sliderlength=10)
 Sa.grid(row=0,column=0,columnspan=2)
 Button(frame1,text="Save",padx=20,command=saveImage).grid(row=1,column=0,padx=5,pady=10)
 Button(frame1,text="Cancel",padx=20,command=cancelFunction).grid(row=1,column=1,padx=5,pady=10)
 Sa.set(50)
 master.mainloop()

def AdjustContrast(c1,image,root,event=None):
 master=Toplevel(root)
 ...
 def saveImage():
  im=Image.open(image)
  enhancer=ImageEnhance.Contrast(im)
  img=enhancer.enhance(scaleRatio(Sb.get()))
  misc.imsave("1.jpg",img)
  master.destroy()
 ...
 Sb=Scale(frame1,length=200,from_=0,to=100,orient=HORIZONTAL,command=getScaleValue,sliderlength=10)
...
def AdjustSharpness(c1,image,root,event=None):
 master=Toplevel(root)
...
def ColorBalancer(c1,image,root,color,event=None):
 master=Toplevel(root)

 def scaleRatio(val):
  return int(val)/50

 def adjustColor(color,factor,img):
  if(color=="RED"): return RGBTransform().mix_with((255,0,0),factor=(factor)).applied_to(img)
  if(color=="GREEN"): return RGBTransform().mix_with((0,255,0),factor=(factor)).applied_to(img)
  if(color=="BLUE"): return RGBTransform().mix_with((0,0,255),factor=(factor)).applied_to(img)
 ...
 def getScaleValue(val):
  ...
  im=Image.open(image)
  img2=im.convert('RGB')
  red=adjustColor(color,val1,img2)
 ...
 def saveImage():
  im=Image.open(image)
  img2=im.convert('RGB')
  img=adjustColor(color,scaleRatio(int(Sb.get())),img2)
  misc.imsave("1.jpg",img)
  master.destroy()
 ...
 Sb=Scale(frame1,length=200,from_=-50,to=50,orient=HORIZONTAL,command=getScaleValue,sliderlength=10)
 Sb.grid(row=0,column=0,columnspan=2)
 Button(frame1,text="Save",padx=20,command=saveImage).grid(row=1,column=0,padx=5,pady=10)
 Button(frame1,text="Cancel",padx=20,command=cancelFunction).grid(row=1,column=1,padx=5,pady=10)
 Sb.set(0)
...
def AdjustGamma(c1,image,root,event=None):
 master=Toplevel(root)
 ...
 def getScaleValue(val):
  ...
  w,h=Image.open(image).size
  # im=Image.open(image)
  im=data.imread(image)
  gamma_image=exposure.adjust_gamma(im,val1)
  misc.imsave("gamma.jpg",gamma_image)
  gamma_image1=Image.open("gamma.jpg")
 ...
 def saveImage():
  im=data.imread(image)
  gamma_image=exposure.adjust_gamma(im,scaleRatio(Sb.get()))
  misc.imsave("1.jpg",gamma_image)
  master.destroy()
...
def AdjustSaturation(c1,image,root,event=None):
 master=Toplevel(root)
 ...
 Sb=Scale(frame1,length=200,from_=0,to=200,orient=HORIZONTAL,command=getScaleValue,sliderlength=10)
 Sb.grid(row=0,column=0,columnspan=2)
 Button(frame1,text="Save",padx=20,command=saveImage).grid(row=1,column=0,padx=5,pady=10)
 Button(frame1,text="Cancel",padx=20,command=cancelFunction).grid(row=1,column=1,padx=5,pady=10)
 Sb.set(100)
...
def ColorCorrection(c1,image,root,event=None):
 ...
 def Ok():
  c1.delete("all")
  w,h=Image.open(image).size
  img=Image.open(image).resize((w,h),Image.ANTIALIAS)
  master.img2=ImageTk.PhotoImage(img)
  c1.create_image(1300/2,(600/2)+10,anchor="center",image=master.img2)
  master.destroy()
...
def ReplaceColor(master,image1,canvas,event=NONE):
 ...
 def replace():
  try:
    ...
    t=int(str(a.get()))
    photo=Image.open("replace_color_image.jpg").resize((350,200),Image.ANTIALIAS)
    ...
    canvas1.delete("all")
    canvas1.create_image(0,0,anchor="nw",image=root.img2)
  except Exception as e:
    messagebox.showinfo("Error",e)
 
 def ok():
  w,h=Image.open(image1).size
  img=Image.open("replace_color_image.jpg")
  img3=img.resize((w,h),Image.ANTIALIAS)
  master.img2=ImageTk.PhotoImage(img3)
  canvas.delete("all")
 ...
 def checkDimensionForOriginalCanvas(image):
  width,height=image.size
 ...
 canvas1.pack(fill=X)
 img=Image.open(image1)
 #newWidth,newHeight=checkDimension(img)
 newWidth=350
 newHeight=200
 img2=img.resize((newWidth,newHeight),Image.ANTIALIAS)
 master.img2=ImageTk.PhotoImage(img2)
 canvas1.create_image(0,0,anchor='nw',image=master.img2) #50,150 because these are our small canvas width/2 ans height/2
 canvas1.bind('<Button-1>',click)
 misc.imsave("replace_color_image.jpg",img2)
...
def ResizeImage(master,image,canvas,event=NONE):
 ...
 # frame2=Frame(root)
 # frame2.pack(side=RIGHT)
 T5=LabelFrame(frame1,text="Some standard dimensions (pixels):",bd=2,relief=GROOVE)
 T5.pack(fill=X)
 ...
 TCan=Frame(frame1)
 TCan.pack()
 Button(TCan,text="Ok",padx=50,underline=0,command=resize1).grid(row=0,column=1,padx=5,pady=5)
