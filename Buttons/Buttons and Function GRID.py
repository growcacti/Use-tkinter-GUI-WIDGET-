...
def open_file():
    file = fd.askopenfilename(
        title="Choose a file of any type", filetypes=[("All files", "*.*")]
    )
...
def copy_file():
    ...
    try:
        ...
        mb.showinfo(
            title="File copied!",
            message="Your desired file has been copied to your desired location",
        )
    except:
        mb.showerror(
            title="Error!",
...
def rename_file():
    file = fd.askopenfilename(
        title="Choose a file to rename", filetypes=[("All files", "*.*")]
    )
    ...
    Label(
        rename_wn,
        text="What should be the new name of the file?",
        font=("Times New Roman", 10),
    ).grid(row=0, column=0)
    new_name = Entry(rename_wn, width=40, font=("Times New Roman", 10))
    new_name.grid(row=1, column=1)
    new_file_name = os.path.join(
        os.path.dirname(file), new_name.get() + os.path.splitext(file)[1]
    )
...
def move_folder():
    ...
    mb.showinfo(
        message="You just selected the folder to move, now please select the desired destination where you want to move the folder to"
    )
    ...
    try:
        ...
        mb.showinfo(
            "Folder moved",
            "Your desired folder has been moved to the location you wanted",
        )
    except:
        mb.showerror(
            "Error",
            "We could not move your folder. Please make sure that the destination exists",
        )
...
Label(r, text=" ", font=("Ariel Black", 15), bg=background, wraplength=250).grid(
    column=0, row=1
)
Label(r, text=title, font=("Ariel Black", 15), bg=background, wraplength=250).grid(
    column=1, row=1
)
Button(
    r,
...
Button(
    r,
    text="List all files in a folder",
    width=20,
    font=button_font,
    bg=button_background,
